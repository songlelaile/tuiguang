  ...
  return (
    <div style={authShell}>
      <form onSubmit={submit} style={authCard}>
        ...
        <a href="#/register" style={authLink}>用邀请码注册 →</a>
      </form>
    </div>
  );
}
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ username, password, email: email || undefined, invite_code: invite })
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) { setErr(json.error || `注册失败 (HTTP ${res.status})`); return; }
      await onAuthChange();
      window.location.hash = "#product";
    } finally { setSubmitting(false); }
  }
  return (
    <div style={authShell}>
      <form onSubmit={submit} style={authCard}>
        ...
        <p style={authSub}>需要管理员发的邀请码才能开通</p>
        <label style={authField}>
          <span style={authLabel}>用户名</span>
          <input style={authInput} autoFocus value={username} onChange={(e) => setUsername(e.target.value)} />
        </label>
